        ...
        if st.button("预览自动生成的简历"):
            ...
            with st.spinner("生成简历中..."):
                response = client.messages.create(
                    model="claude-haiku-4-5-20251001",
                    max_tokens=2048,
                    system="""你是一个专业的简历撰写顾问。根据用户的经验库，生成一份格式标准的简历。
        ...
        if not jd_input:
            ...
            with st.spinner("AI分析中..."):
                response = client.messages.create(
                    model="claude-haiku-4-5-20251001",
                    max_tokens=2048,
                    system="""你是一个专业的简历优化顾问。只做简历优化，不涉及面试准备。
        ...
        if not interview_jd:
            ...
            with st.spinner("AI预测面试问题中..."):
                response = client.messages.create(
                    model="claude-haiku-4-5-20251001",
                    max_tokens=2048,
                    system="""你是一个资深面试官。根据JD和候选人简历，预测8个面试问题。
...
包含：行为类（2题）、能力类（2题）、情景类（2题）、动机类（1题）、压力测试类（1题）。用中文。""",
            ...
            if not practice_question or not practice_answer:
                ...
                with st.spinner("AI点评中..."):
                    feedback_response = client.messages.create(
                        model="claude-haiku-4-5-20251001",
                        max_tokens=1024,
                        system="""你是一个资深面试教练。评估候选人的回答思路，给出：
1. 亮点
2. 不足
3. 优化建议
        ...
        if st.button("结束并生成总结"):
            with st.spinner("生成总结中..."):
                summary_response = client.messages.create(
                    model="claude-haiku-4-5-20251001",
                    max_tokens=1024,
                    system="你是一个简历顾问，根据对话内容生成结构化的用户信息总结，分为：基础信息、技能、经历片段、性格与软实力四个类别。用中文，简洁清晰。",
                    messages=st.session_state.onboarding_messages + [
